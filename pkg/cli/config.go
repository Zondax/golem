...
func SetupConfiguration(c *cobra.Command) {
	...
	if err != nil {
		logger.Log().Fatalf(context.Background(), "unable to bind config flag: %+v", err)
	}
...
func LoadConfig[T Config]() (*T, error) {
	...
	if configFileOverride != "" {
		...
		logger.Log().Infof(context.Background(), "Using config file: %s", viper.ConfigFileUsed())
	}

	err = viper.ReadInConfig()
	if err != nil {
		logger.Log().Fatalf(context.Background(), "%+v", err)
	}

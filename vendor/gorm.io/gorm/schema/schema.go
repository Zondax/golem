...
func ParseWithSpecialTableName(dest interface{}, cacheStore *sync.Map, namer Namer, specialTableName string) (*Schema, error) {
	...
	if value.Kind() == reflect.Ptr && value.IsNil() {
		fmt.Println("Destination is nil, initializing it...")
		value = reflect.New(value.Type().Elem())
	}
	if value.Kind() != reflect.Ptr && value.IsZero() {
		fmt.Println("Destination is zero value, initializing it...")
	...
	if modelType.Kind() != reflect.Struct {
		fmt.Printf("Error: Unsupported data type. The type of 'dest' is: %+v\n", modelType)
		if modelType.PkgPath() == "" {
			fmt.Printf("Error: Unsupported data type. The type of 'dest' is: %v\n", value.Type())
			fmt.Printf("Error: Unsupported data type. The value of 'dest' is: %+v\n", value)
			value = value.Elem()
			fmt.Printf("Error: Unsupported data type. The value of 'dest' is: %+v\n", value.Interface()) // Dereference the pointer
			return nil, fmt.Errorf("%w: %+v %s", ErrUnsupportedDataType, dest, modelType.Name())
		}
